...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  private setupAnimations(): void {
    ...
    // Create stun animation from stun frames
    if (!anims.exists("stun_animation")) {
      const stunFrames: any[] = []
      for (let i = 1; i <= 10; i++) {
        stunFrames.push({ key: `stun_frame1_${i}`, duration: 80 })
      }
      anims.create({
        key: "stun_animation",
        frames: stunFrames,
  ...
  private handleStunnedState(): void {
    ...
    // Stun animation with flashing effect
    this.setTint(0xff6666)
    
    // Play stun animation if available
    if (this.scene.anims.exists("stun_animation")) {
